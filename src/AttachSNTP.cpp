...
{
   // Инициируем интервал синхронизации времени
   SynchroMIN=smin;
   // Инициируем временную зону (часовой пояс)
   Timezone=zone;
   ...
   {
      Serial.println("Время еще не установлено. Получаем время по протоколу SNTP");
      initSNTP();
      wait4SNTP();
      time(&now);
   }
...
{
   Serial.println("");
   Serial.println("Время синхронизировано с сервером точного времени!");
}
// ****************************************************************************
...
{
   Serial.print("Ожидание синхронизации .");
