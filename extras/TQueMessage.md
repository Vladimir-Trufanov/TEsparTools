## [TQueMessage - обеспечить передачу и приём сообщений через очередь в задачах и из прерываний](#) v3.3.0, 09.03.2025 
...
tmessAPP amessAPP[] = 
{
    ...
    {SendStrExt,   tvm_3strmes, "Cообщение c уточнением %s"},  // "Cообщение c уточнением типа String"
};
...
- ***Отправить просто сообщение, без уточнений*** 
...
-  ***Отправить сообщение c одним уточнением типа String (31 символ)***

Указанным методом выполняется отправка сообщения в очередь из задачи с использованием дополнительного параметра типа String (31 символ) ***fmess32***. 
