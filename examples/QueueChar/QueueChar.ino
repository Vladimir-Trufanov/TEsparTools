...
//   которое обрезается до 1023 символов с завершающим нулем;
...
void loop() 
{
   ...
   String inMess;
   inMess="Максимально длинное сообщение получается из 1023 байт [буфер текстов сообщений максимально может содержать 1023 байта и завершающий ноль 1234567890]\n";
   inMess += inMess;  
   inMess += inMess;  
   inMess=queMessa.Send(inMess);
   if (inMess!=tisOk) Serial.println(inMess); 
}
